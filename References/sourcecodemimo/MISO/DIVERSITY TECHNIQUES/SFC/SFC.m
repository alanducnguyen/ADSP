...
%%-----Channel-h1-----%%
  ...
  %%-----Channel-h2------%%
        ...
        h22=fft((alpha_e_ph2),N/2)/sqrt(N/2);%%---CHANNEL_2 FFT---%%
        for i=1:1:N/2 
 ...
 %%-----Received Signal-----%%
  ...
  %%---Signal Estimation from Received Signal---%%  
   ...
   %%---Estimated Signal Decoding---%%
    ...
    Pb_ASFC(Eb_No_dB+1)=mean(Ber);
end        
Eb_No_dB=0:1:12;
semilogy(Eb_No_dB,Pb_ASFC,'r--');
grid on;
hold on;
axis([0 12 10^-4 0.5]);
legend('Alamouti Space Frequency Coding');
%legend('ASTBC','ASFBC');
xlabel('SNR(dB)');
ylabel('BER');
title('Alamouti Space Frequency Coding (MT=2,MR=1)in Rayleigh fading channel');
