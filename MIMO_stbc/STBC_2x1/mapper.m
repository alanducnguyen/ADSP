function [ qamSymbols ] = mapper( tx_bits,b,N_frame,is_convolution_encoding)
...
%   2. we loop each row to read the bits value in row
    if strcmp(is_convolution_encoding,'on')
        extra_bit = 2;
    else
        extra_bit = 1;
    end    
    
    reshapeMatrix = reshape(tx_bits,[b,N_frame*extra_bit]);
